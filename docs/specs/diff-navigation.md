...
| EXP-3 | Each Enter/Space press on a bar advances the expansion by exactly one chunk (100 lines) in the bar's direction(s) — identical to what the renderer reveals for the same action. | `both` advances both ends by 100. Map consumption is exercised by the EXP-1/2 unit tests; the renderer lockstep is e2e. | e2e (expansion.e2e.ts) |
...
| EXP-5 | Shift+click on any expand control, and the "Expand all" button, reveal the bar's entire hidden range in one action. | Recorded as `Infinity`; clamping makes any over-expansion equivalent to full reveal. | unit (clamp, diffNav.test.ts) + e2e (expansion.e2e.ts) |
...
| EXP-7 | EXP-6 holds identically when the expansion came from a mouse click on the bar's pill or expand buttons (pill = the bar's own direction(s); each button = its direction). | Clicks are handled inside the diff library; the nav model mirrors them. | e2e (expansion.e2e.ts) |
| EXP-8 | Expanding a bar (any entry point) never changes the line selection. | A selection made for a comment must survive expansion. | e2e (expansion.e2e.ts) |
...
| EXP-10 | j/k never lands on a bar that no longer renders (fully revealed). | Fully revealed gaps leave the stop list; a virtualization-recycled bar that still exists is a valid landing target. | e2e (expansion.e2e.ts) |
...
| ID | Behavior (MUST) | Notes | Covered by |
|----|-----------------|-------|------------|
| NAV-1 | `j` moves the cursor to the next stop; `k` to the previous stop. Landing on a line stop selects exactly that line on the stop's side and re-anchors there. | | e2e (navigation.e2e.ts, smoke.e2e.ts) |
| NAV-2 | j/k does not wrap: `j` on the last stop and `k` on the first stop do nothing. | | e2e (navigation.e2e.ts) |
| NAV-3 | Bars are stops: landing on one highlights the bar's pill(s) and leaves the existing line selection untouched. | The bar cursor is the visible cursor while on a bar. | e2e (smoke.e2e.ts) |
| NAV-4 | `]` jumps to the start of the next change block and `[` to the previous one, wrapping around the file in both directions. | | e2e (navigation.e2e.ts) |
...
| NAV-6 | Navigation keys are inert while focus is in an input, textarea, or contenteditable, and while Cmd/Ctrl/Alt is held. (Shift is meaningful: Shift+J/K.) | | e2e (navigation.e2e.ts) |
| NAV-7 | Navigation keys act on the newly selected file immediately after a file switch — no click required first. | | e2e (navigation.e2e.ts) |
...
| ID | Behavior (MUST) | Notes | Covered by |
|----|-----------------|-------|------------|
| CUR-1 | A visible bar cursor outranks the line selection: j/k steps from the bar. | | e2e (navigation.e2e.ts) |
...
| ID | Behavior (MUST) | Notes | Covered by |
|----|-----------------|-------|------------|
| SEL-1 | Clicking a line selects exactly that line on the clicked side and sets the anchor there. | | e2e (selection.e2e.ts) |
| SEL-2 | Shift+click extends the selection from the anchor to the clicked line (anchor unchanged). | | e2e (selection.e2e.ts) |
| SEL-3 | Dragging across the code area selects the range from the pointer-down line to the line under the pointer, updating live during the drag. | | e2e (selection.e2e.ts) |
| SEL-4 | Shift+J/K moves the selection's **end** to the adjacent line stop, skipping bars, while the anchor stays fixed. Shrinking past the anchor is allowed (end may pass to the other side of it). | | e2e (selection.e2e.ts) |
| SEL-5 | Plain j/k after a multi-line selection collapses it to a single line: the stop adjacent to the selection's end. | | e2e (selection.e2e.ts) |
| SEL-6 | Escape closes the comment composer and clears the line selection. | | e2e (selection.e2e.ts) |
| SEL-7 | Making a line selection (click, drag, Shift+J/K, or j/k landing) clears any bar-cursor highlight. | | e2e (selection.e2e.ts) |
| SEL-8 | Switching files clears the selection and any open composer draft. | | e2e (selection.e2e.ts) |

## SCR — scrolling and visibility

| ID | Behavior (MUST) | Notes | Covered by |
|----|-----------------|-------|------------|
| SCR-1 | Every keyboard cursor move issues exactly one scroll request, targeting the destination row with align-nearest semantics, a 40 px margin, and instant behavior. | Observable via the `[nav] scrollTo` log: one entry per keypress. | e2e (scrolling.e2e.ts) |
| SCR-2 | No scroll occurs when the destination row is already fully visible with the margin — repeated j/k inside the viewport leaves scrollTop untouched until the cursor nears an edge. | | e2e (scrolling.e2e.ts) |
| SCR-3 | `]`/`[` to an off-screen change block lands with the cursor row inside the viewport (within margin of an edge at worst). | | e2e (scrolling.e2e.ts) |
| SCR-4 | Expanding a bar issues no cursor scroll, and the rows visible above the bar keep their on-screen position — revealed lines appear in place at the bar's location, never teleporting the viewport. | The renderer anchors the relayout; we must not fight it. | e2e (scrolling.e2e.ts) |
...
| ID | Behavior (MUST) | Notes | Covered by |
|----|-----------------|-------|------------|
| FILE-1 | File switch resets the cursor, anchor, and bar cursor (selection clears per SEL-8). Expansion state persists per EXP-11. | *(Amended in review: expansion no longer resets here.)* | e2e (selection.e2e.ts) |
| FILE-2 | Toggling split↔unified preserves the line selection, and the cursor resolves to the same file line in the new layout's stop list. | Stop indices differ between modes; the line must not. | e2e (files.e2e.ts) |
| FILE-3 | For a file present in both buckets, the Unstaged/Staged toggle swaps to the other entry's contents with a fresh view (top scroll, reset nav state). | | e2e (files.e2e.ts) |
