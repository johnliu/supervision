// Install the supervision-feedback skill for Claude Code. The repo's
// skills/supervision-feedback/SKILL.md is the single source of truth: Bun
// inlines it here at build time (text import — verified to survive `bun
// build`), and installing writes it to the user-level skills directory, where
// Claude Code picks it up in every project.
...
import skillMarkdown from '../../skills/supervision-feedback/SKILL.md' with { type: 'text' };
import type { SkillStatus } from '../shared/types';

const SKILL_TEXT: string = skillMarkdown;

function skillPath(): string {
  return path.join(homedir(), '.claude', 'skills', 'supervision-feedback', 'SKILL.md');
}
